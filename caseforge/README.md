...
| `catalog.py` | Erzeugt die Artefaktübersicht (`examples/operation_waldweg/06_master/Artefakt_Katalog.*`). |
...
Die eigentlichen Generatoren bleiben in `caseforge/generators/` und werden über die Registry angesteuert.
...
Jeder Registry-Eintrag nennt sein **Gegenprüf-Tool**. `forge.py validate` fährt die Gates; lokal zusätzlich die echten Tools laut `docs/Toolchain_und_Bewertung_Dozent.md` (iLEAPP, ALEAPP, RegRipper, EvtxECmd, Hindsight, LnkParse3, BIOME-Stream-Analyzer). Dokumentierte Grenzen: SRUM/ESE, $MFT/$UsnJrnl, valides Prefetch-SCCA, ShimCache.
